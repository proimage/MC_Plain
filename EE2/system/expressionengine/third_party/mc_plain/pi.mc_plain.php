...
class Mc_Plain
{
	...
	function email($member_id = '')
	{
		...
		else
		{
			...
			$this->DB->where('member_id', $this->DB->escape_str($member_id));
	...
	function field($member_id = '',$field_name = '')
	{
		...
		{
			// Build SQL query
			$this->DB->select('m_field_id');
			$this->DB->where('m_field_name', $this->DB->escape_str($field_name));
			...
			$this->DB->where('member_id', $this->DB->escape_str($member_id));
			$query = $this->EE->db->get('exp_member_data');
...
/* Location: /system/expressionengine/thirdparty/mc_plain/pi.mc_plain.php */